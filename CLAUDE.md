...
| `static/vk-atelier-staff.css` | Tonal overrides of core vraiKronos components (drawer, tables, tags, fields, dialogs, KPI tiles; tabs are the shared `.vk-tabs` in `vk-atelier-components.css`) + `st-*` layout classes for the two staff pages; scoped to `[data-theme="atelier"]`, not linked by the student app |
| `static/brand-mark.svg` | The vraiFrench logo (helium atom), single colour; pages show it via `.pa-brand-mark` (CSS mask tinted by `--vk-brand-mark`). XML file: never put a double hyphen in its comments |
| `static/pa-voice-atom.js` | `PaVoiceAtom` — the brand mark as a canvas voice indicator: rest (logo), `voice('audio'|'you', env)` blue/gold waves across, `startLoading()`/`stopLoading()` collision loader; `PaVoiceAtom.speechEnvelope(text, ms)`. Landing page only: hero demo (mic → mark → gold waves → loader, between the phrase card and the speech tile; blue audio waves were dropped there as a distraction) and How it works (Listen = play → mark → blue waves, Speak = mic → mark → gold waves). Same file: `PaOrbitRing` — the record button as the atom. The phrase record button is a plain circle (`.pa-ctrl-slot.is-orbit`); while recording, two gold electrons orbit it and draw the orbit as a ring on `.pa-rec-orbit`, melting into it as it closes; the ring thickens with the mic level (second audio stream on desktop, a steady simulated level on phones). Wired by `_initRecordOrbit()` watching the button's `.listening` class |
...
- Progress & loading (M3): every bar is `.pa-lp` + `.pa-lp-fill` (4px, gap between fill and track, stop dot; `--pa-lp-h: 8px` for thick); rings are `.pa-cp` (JS sets `--pa-cp-pct` 0–1 on the `<svg>`); loading states use `.pa-loading` + an empty `<span class="pa-loader">` (the brand-mark atom loader; `paLoaderSvg()` fills it, since each copy needs its own mask id) with `toggleLoading()` / `showLoadingError()` / `loadingHtml()`; when a control waits on its own content (the phrase play button), wrap loader + control in `.pa-ctrl-slot` and toggle `.is-loading` (`_setPhraseLoading()`). Never hand-roll a track div
- Brand mark: the helium atom (`static/brand-mark.svg`), shown with `.pa-brand-mark` (CSS mask tinted by `--vk-brand-mark`), sized on the element. Used in the rail, the staff sidebars and the leftover gap in the Home tiles (`_homeFillBrandTile()`)
- Buttons: always `.vk-btn` + a variant — `-primary` (filled) · `-tonal` · `-outline` · `-text` · `-ghost` · `-destructive`; sizes `.sm` / default / `.lg` (48px hub start). The Tonal versions of these live in `vk-theme-atelier.css` and apply on every page; never hand-roll a page-level button. Icon-only secondary actions (close ✕, replay) are `.vk-icon-btn` (`.is-sm`, `.is-tonal`); play/record stay `pa-ctrl`. Single-select toggles are `.vk-seg` segmented buttons (`.is-block` to stretch). In-page view switches are M3 `.vk-tabs` / `.vk-tab` (Practice list; `aria-selected`/`.active`, `.is-block` to stretch). Speaking's Phrases and Paragraphs are separate area tabs (`phrase-hub` / `paragraph-hub` views) sharing one hub page, `#phrase-hub`; `switchView` sets its `data-mode`, and anything marked `data-only="phrase|paragraph"` (intro, Sound focus, paragraph-only styles) shows on that tab only Filter/picker chips are `.vk-filter-chip` (selected: `.is-active`, `.active`, or a checked inner checkbox)
